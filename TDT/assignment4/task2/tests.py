...
def test_iou():
    ...
    ans = 0
    assert res == ans, "1Expected {}, got: {}".format(ans, res)
    ...
    res = calculate_iou(b1, b2)
    ans = 1/7
    assert res == ans, "2Expected {}, got: {}".format(ans, res)

    b1 = np.array([0, 0, 1, 1])
    b2 = np.array([0, 0, 1, 1])
    res = calculate_iou(b1, b2)
    ans = 1.0
    assert res == ans, "3Expected {}, got: {}".format(ans, res)

    b1 = np.array([0, 0, 1, 1])
    b2 = np.array([0.5, 0.5, 1, 1])
    res = calculate_iou(b1, b2)
    ans = 0.25
    assert res == ans, "4Expected {}, got: {}".format(ans, res)

    b1 = np.array([5.5, 5.5, 8, 8])
    b2 = np.array([5.5, 3, 8, 4])
    res = calculate_iou(b1, b2)
    ans = 0.0
    assert res == ans, "5Expected {}, got: {}".format(ans, res)

    b1 = np.array([5.5, 5.5, 8, 8])
    b2 = np.array([3, 5.5, 4, 9])
    res = calculate_iou(b1, b2)
    ans = 0.0
    assert res == ans, "6Expected {}, got: {}".format(ans, res)

    b1 = np.array([522, 540, 576, 660])
    b2 = np.array([520, 540, 570, 655])
    res = round(calculate_iou(b1, b2), 5)
    ans = 0.82265
    assert res == ans, "7Expected {}, got: {}".format(ans, res)
...
def test_calculate_precision_recall_all_images():
    ...
    assert res1 == ans1, "1Expected {}, got: {}".format(ans1, res1)
    assert res2 == ans2, "2Expected {}, got: {}".format(ans2, res2)
