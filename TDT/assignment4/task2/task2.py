...
def calculate_iou(prediction_box, gt_box):
    ...
    #print("pred, gt, common, union")
    #print(Area_pred,Area_gt,Area_union,iou)
...
def get_precision_recall_curve(all_prediction_boxes, all_gt_boxes, confidence_scores, iou_threshold):
    ...
    for threshold in confidence_thresholds: # 0.0 0.001 0.002 ... 0.9999
        ...
        for i in range(len(all_prediction_boxes)): #0 1
            ...
            matched_pred_boxes, matched_gt_boxes = get_all_box_matches(pred_boxes[indices], gt_boxes, iou_threshold)

            #print(pred_boxes[indices])
            # Compute number of true/false positives/negatives
            num_matched = len(matched_pred_boxes)
            num_gt = len(gt_boxes)
            ...
            num_false_positives = len(indices) - num_matched
...
def calculate_mean_average_precision(precisions, recalls):
   ...
   # Calculate the mean average precision given these recall levels.
    ...
    for level in recall_levels:
        ...
        if max_precision:
            average_precision += max_precision / len(recall_levels)
