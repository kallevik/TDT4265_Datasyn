...
def compute_loss_and_accuracy(
    ...
    with torch.no_grad():
        for (X_batch, Y_batch) in dataloader:
            ...
            accuracy += torch.sum(predicted_class==Y_batch).item()
            total_sampels += X_batch.size(0)
    ...
    #print("tot_sampels: ", total_sampels)
    
    average_loss = total_loss / i #len(dataloader.dataset)
    
    #accuracy /= len(dataloader.dataset)
    #accuracy = 100* accuracy / len(dataloader.dataset)    
    accuracy = accuracy / total_sampels 
...
class Trainer:
    ...
    def __init__(self,
        ...
        optimizer_dict={
            'SGD':torch.optim.SGD(self.model.parameters(),self.learning_rate, weight_decay=self.l2_reg_lambda, momentum =self.momentum),
            'Adam':torch.optim.Adam(self.model.parameters(),self.learning_rate, weight_decay=self.l2_reg_lambda)}
        
        self.optimizer = optimizer_dict.get(optimizer)
        
        #self.optimizer = torch.optim.SGD(self.model.parameters(),self.learning_rate, weight_decay=self.l2_reg_lambda)
        #self.optimizer = torch.optim.Adam(self.model.parameters(),self.learning_rate, weight_decay = self.l2_reg_lambda)
    ...
    def train_step(self):
        """
        Computes the loss/accuracy for the train dataset
        """
        return 1
    ...
    def validation_step(self):
        ...
        print(
            ...
            f"Bps: {self.global_step / used_time:.2f}",
            f"Global step: {self.global_step:>6}",
            f"\n Train Loss: {train_loss:.2f}",
            f"Train Acc: {train_acc:.3f}",
            f"Val Loss: {validation_loss:.2f}",
            f"Val Acc: {validation_acc:.3f}",
