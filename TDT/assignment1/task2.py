...
def calculate_accuracy(X: np.ndarray, targets: np.ndarray, model: BinaryModel) -> float:
    ...
    accuracy = np.sum(outputs>=0.5)/targets.shape[0]
    print(accuracy)
    #accuracy = 0.965#0.0
...
class LogisticTrainer(BaseTrainer):
    ...
    def train_step(self, X_batch: np.ndarray, Y_batch: np.ndarray):
        ...
        Args:
            X: one batch of images [128,785]
            Y: one batch of labels [128,1]
        Returns:
        ...
        #print("outputs.shape: ",outputs.shape) #(128,1)
        #print("w.shape: ", self.model.w.shape) #(785,1)
        
        #with open('_outputs.txt', 'a') as f:
        #    print('Outputs.T:', outputs.T, file=f)  # Python 3.x

        #with open('_loss.txt', 'a') as f:
        #    print('Loss:', loss, file=f)  # Python 3.x

        #with open('_Y_batch.txt', 'a') as f:
        #    print('y_batch.T:', Y_batch.T, file=f)  # Python 3.x
...
def main():
    ...
    # Plot accuracy
    plt.ylim([0.0, 1.10])#plt.ylim([0.93, .99])
